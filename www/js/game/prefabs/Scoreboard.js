...
Scoreboard.prototype.show = function(score) {
  ...
  gameoverText = this.game.add.bitmapText(0,40, 'minecraftia', 'You Died.', 30);
  gameoverText.x = this.game.width/2 - (gameoverText.textWidth / 2);
  this.add(gameoverText);

  scoreText = this.game.add.bitmapText(0, 100, 'minecraftia', 'Your Score: ' + score, 20);
  scoreText.x = this.game.width / 2 - (scoreText.textWidth / 2);  
  this.add(scoreText);

  highScoreText = this.game.add.bitmapText(0, 140, 'minecraftia', 'Your High Score: ' + highscore, 20);
  highScoreText.x = this.game.width / 2 - (highScoreText.textWidth / 2);  
  this.add(highScoreText);

  startText = this.game.add.bitmapText(0, 200, 'minecraftia', 'Tap to play again!', 16);
  startText.x = this.game.width / 2 - (startText.textWidth / 2);  
  this.add(startText);

  if(isNewHighScore) {
    newHighScoreText = this.game.add.bitmapText(0, 100, 'minecraftia', 'New High Score!', 10);
